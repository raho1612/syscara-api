...
from core.database import get_cached_or_fetch, fetch_live_then_cache, iter_items, load_from_supabase_chunked
...
def register_vehicle_routes(app):
    ...
    def api_stats():
        use_stale = str(request.args.get('allow_stale', '0')).lower() in ('1', 'true', 'yes')
        try:
            raw = fetch_live_then_cache('sale/vehicles', f"{SYSCARA_BASE}/sale/vehicles/", allow_stale_fallback=use_stale)
